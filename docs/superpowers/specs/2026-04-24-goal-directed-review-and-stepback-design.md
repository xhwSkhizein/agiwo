...
> 替换现有的 token/round-count-based tool-result rewrite 流程，引入基于目标对齐度的 review 机制。
> 解决三大问题：System prompt/tool schema 未对齐、频繁提醒成为噪音、message offload 破坏 KV cache。

## Problem

当前旧的 tool-result rewrite 流程有 3 个根本性问题：

1. **Review 没有语义锚点**：基于 token 计数和 step 轮次触发，不关心 agent 是否真的偏离目标。agent 做有用工作时也可能反复触发，成为噪音。

2. **System prompt 与 tool schema 割裂**：system prompt 完全不提 `<system-notice>` 或这套 rewrite 流程，对应的 tool schema 又是孤立的。agent 不理解 system-notice 的权威性和处理优先级。

3. **KV cache 破坏**：旧的 message-rewrite helper 会 deep-copy 整个 message list，替换所有 tool content 为 placeholder，删除临时 review tool_call + result，然后 `rebuild_messages` 重建。代价是 KV cache 全量失效 + 重新编码的 token 成本。

## Design

### 1. 核心思路

从 **"按 token 计数触发 → agent 主动总结 tool 结果 → 系统 offload"** 转变为 **"基于目标对齐的系统 review → agent 确认/否认 → 系统执行 targeted content 替换"**。
...
去掉 `review_step_interval`、`review_step_interval`、`review_step_interval`。
...
| | 当前 StepBack | 新 StepBack |
|---|---|---|
| Message 删除 | 删除 step-back 自身的 tool_call + tool_result | prompt-visible 上隐藏 review_trajectory 自身的 tool_call + tool_result；run-log 仍保留 |
...
| 删除/替换 | 说明 |
|-----------|------|
| `agiwo/agent/review/` 整个包 | 替换为 `GoalManager` + `ReviewEnforcer` + `StepBackExecutor` |
| `ReviewTrajectoryTool` | 替换为 `declare_milestones` + `review_trajectory` |
| 旧的 review-tracking state | 替换为 `Milestone[]` + `ReviewCheckpoint` |
| 4 个 step-back 配置字段 | 替换为 `review_step_interval` + `review_on_error` |
| Repo guard AGW045-049 | 更新为新模块的对应规则 |
| `BEFORE_REVIEW` / `AFTER_STEP_BACK` hook phases | 替换为 `BEFORE_REVIEW` / `AFTER_STEP_BACK` |
| `StepBackApplied` log/stream 类型 | 替换为 `StepBackApplied` |
| Console step-back 展示逻辑 | 替换为 step-back 展示 |
| Console step-back 配置表单 / TS payload types | 删除并替换为 review 配置字段 |
| 现行文档与 archived 文档中的 step-back 术语 | 全量替换为 review / step-back 语义，并在需要处标注“已被替代” |
| 旧测试 `test_step-back.py` 等 | 替换为新测试 |
...
| `agiwo/agent/run_tool_batch.py` | `ReviewBatch` → `ReviewBatch`，去掉 `rebuild_messages`，统一 review metadata cleanup outcome |
| `agiwo/agent/models/run.py` | `RunLedger` 上新字段收敛到 `ReviewState`（含 `latest_checkpoint` / `pending_review_reason`） |
| `agiwo/agent/models/config.py` | 替换 4 个 step-back 字段为 review 配置 |
| `agiwo/agent/models/log.py` | `StepBackApplied` → `StepBackApplied`，新增 hidden-from-context fact（如 `ContextStepsHidden`） |
| `agiwo/agent/models/stream.py` | `StepBackAppliedEvent` → `StepBackAppliedEvent` |
| `agiwo/agent/hooks.py` | `BEFORE/AFTER_STEP_BACK` → `BEFORE_REVIEW` / `AFTER_STEP_BACK`，并补齐真实调用点 |
...
| `console/web/src/components/agent-form.tsx` | 删除旧 step-back 配置，补齐真实 review 配置 |
| `console/web/src/components/session-detail/session-observability-panel.tsx` | 统一展示 `step_back`，移除 `step-back` 分支 |
| `console/web/src/lib/api.ts` | TS payload / event kind 与后端真实字段对齐 |
| `docs/**` | 清理现行与 archived 文档中的旧 step-back 术语 |
...
| 模块 | 说明 |
|------|------|
| `agiwo/agent/review/` | 整个包 |
| `agiwo/scheduler/runtime_tools.py::ReviewTrajectoryTool` | 类删除 |
