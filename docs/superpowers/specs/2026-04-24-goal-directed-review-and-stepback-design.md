...
| 触发条件 | 逻辑 | 意图 |
|---------|------|------|
| **Milestone 切换** | Agent 标记 milestone 完成并激活下一个 | 自然检查点 |
...
Recent trajectory:
  - {tool_name}({args_summary}) → {outcome_summary}
  - ...
...
**只修改 tool result message 的 content 字段**，不删除、不重排、不新增任何 message。

| | 当前 Retrospect | 新 StepBack |
|---|---|---|
| Message 删除 | 删除 retrospect 自身的 tool_call + tool_result | 仅删除 review_trajectory 自身的 tool_call + tool_result |
| Content 替换 | 替换为 `[offloaded to disk]` placeholder | 替换为 `[EXPERIENCE] ...` |
| 业务 message 变更 | 可能改变 message 顺序（删除 + 重排） | 不删除不新增，只改 content 字段 |
| KV cache 影响 | 删除 + 重排导致全量失效 | review_trajectory 的 2 条 msg 在末尾，删除不影响前缀；业务 msg 的 content 替换只影响替换点之后 |
...
async def _apply_step_back_outcome(context, outcome, writer):
    if not outcome.applied:
        return
    # Targeted content update，不重建 message list
...
- 不涉及 backend service 或 API schema 变更
...
| `agiwo/agent/run_tool_batch.py` | `RetrospectBatch` → `ReviewBatch`，去掉 `rebuild_messages` |
| `agiwo/agent/models/run.py` | `RunLedger` 上新字段：`milestones`, `review_checkpoint` |
| `agiwo/agent/models/config.py` | 替换 4 个 retrospect 字段为 review 配置 |
| `agiwo/agent/models/log.py` | `RetrospectApplied` → `StepBackApplied` |
| `agiwo/agent/models/stream.py` | `RetrospectAppliedEvent` → `StepBackAppliedEvent` |
| `agiwo/agent/hooks.py` | `BEFORE/AFTER_RETROSPECT` → `BEFORE_REVIEW` / `AFTER_STEP_BACK` |
