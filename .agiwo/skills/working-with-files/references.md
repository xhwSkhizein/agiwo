...
This skill is based on context engineering principles from Manus, the AI agent company leading ahead.

## The 4 Manus Principles
...
> "Creates and updates `task_plan.md` throughout tasks to push global plan into model's recent attention span."

**Problem:** After ~50 tool calls, we may forget original goals ("lost in the middle" effect).
...
**Key Insight:** Manus originally used `task_plan.md` for task planning but found ~33% of actions were spent updating it. Shifted to dedicated planner agent calling executor sub-agents.
