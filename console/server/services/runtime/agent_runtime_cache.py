"""Runtime MainAgent cache and lifecycle management for session-bound execution."""
...
from agiwo.agent import Agent, MainAgent, MainAgentState
...
from server.services.runtime.agent_factory import materialize_main_agent
...
class AgentRuntimeCache:
    """Cache for session-bound MainAgent instances keyed by session_id."""
    ...
    @property
    def main_agents(self) -> dict[str, MainAgent]:
        return {key: cached.main_agent for key, cached in self._cache.items()}

    @property
    def runtime_agents(self) -> dict[str, Agent]:
        """Underlying SDK Agents for diagnostics and legacy call sites."""
        return {key: cached.main_agent.agent for key, cached in self._cache.items()}
    ...
    async def _get_or_create_main_agent_locked(self, session: Session) -> MainAgent:
        """Helper method that performs the actual get/create/refresh logic under lock."""
        ...
        if cached is not None:
            if cached.config_snapshot == snapshot:
                return cached.main_agent
            if cached.main_agent.state is MainAgentState.RUNNING:
                logger.info(
                    "runtime_main_agent_refresh_deferred",
                    session_id=session.id,
                    base_agent_id=session.base_agent_id,
                    reason="main_agent_running",
                )
                return cached.main_agent
            logger.info(
                "runtime_main_agent_refresh_on_config_change",
                session_id=session.id,
                base_agent_id=session.base_agent_id,
            )
        ...
        if retired is not None:
            await retired.main_agent.close()

        worker_scheduler = (
            scheduler_worker_port(self._scheduler)
            if self._scheduler is not None
            else None
        )
        main_agent = await materialize_main_agent(
            base_config,
    ...
    async def get_or_create_main_agent(self, session: Session) -> MainAgent:
        """Get or create a MainAgent for the given session, with per-session locking."""
        lock = self._get_lock(session.id)
        async with lock:
            return await self._get_or_create_main_agent_locked(session)
    ...
    async def close(self) -> None:
        close_tasks = [cached.main_agent.close() for cached in self._cache.values()]
        if close_tasks:
