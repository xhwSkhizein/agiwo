...
and streaming raw AgentStreamItem events back to the channel.

State resolution logic:
    None / COMPLETED / FAILED  →  submit new persistent agent
    IDLE / FAILED (persistent) →  enqueue input to existing agent
    RUNNING / WAITING / QUEUED →  steer (inject message, yield SteerAccepted)
...
class SteerAccepted:
    """Marker yielded when user input is steered into a running agent."""

    state_id: str


ExecutorOutput = Union[AgentStreamItem, SteerAccepted]
...
class AgentExecutor:
    ...
    async def execute(
        ...
        """Resolve scheduler state, submit/enqueue/steer, and yield raw events."""
        ...
        if status in (
            ...
            yield SteerAccepted(state_id=session.scheduler_state_id)
            return

        if status == AgentStateStatus.PENDING:
            async for item in self._handle_pending(agent, session, user_input):
                yield item
            return
        ...
        if use_enqueue:
            async for item in self._enqueue_and_stream(agent, session, user_input):
                yield item
        else:
            async for item in self._submit_and_stream(agent, session, user_input):
                yield item
    ...
    async def _submit_and_stream(
        ...
        async for item in self._scheduler.stream(
            user_input,
            agent=agent,
