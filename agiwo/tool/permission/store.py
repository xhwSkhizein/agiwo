"""
Consent storage and wait coordination for tool permission system.

Stores user consent records (allow/deny patterns) with expiration support,
and coordinates waiting for user consent decisions using asyncio.Event.
"""
...
# ── ConsentStore ABC + Implementations ───────────────────────────────────
...
class MongoConsentStore(ConsentStore):
    ...
    async def save_consent(
        ...
        """Save consent record"""
        await self._ensure_initialized()
        normalized_deny_patterns = [] if deny_patterns is None else deny_patterns
                    ...
                    query,
                        {
                            "$set": {
                                "patterns": patterns,
                                "deny_patterns": normalized_deny_patterns,
